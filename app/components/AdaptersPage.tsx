...
export function AdaptersPage(): React.ReactElement {
  ...
  useEffect(() => {
    if (wasDownloading.current && !downloading) window.dispatchEvent(new CustomEvent('mm3:adapters-changed'));
  ...
  const importFiles = async (files: File[]) => {
    ...
    try {
      ...
      setImportName('');
      setTab('installed');
      window.dispatchEvent(new CustomEvent('mm3:toast', { detail: { message: t('adaptersImported'), type: 'success' } }));
      window.dispatchEvent(new CustomEvent('mm3:adapters-changed'));
      await refresh();
    } catch (problem) {
