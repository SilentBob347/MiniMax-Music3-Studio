  ...
  useEffect(() => {
    const read = () =>
      void fetchAdapters()
        ...
        .catch(() => undefined);
    read();
    window.addEventListener('mm3:adapters-changed', read);
    window.addEventListener('mm3:models-changed', read);
    return () => {
      window.removeEventListener('mm3:adapters-changed', read);
      window.removeEventListener('mm3:models-changed', read);
    };
  }, []);
  return library;
}
