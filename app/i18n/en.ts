export const en = {
    ...
    computeBackendHint_auto: 'CUDA on an NVIDIA card, the processor if CUDA does not work there; Vulkan on AMD and Intel cards (experimental), then the processor. A device that fails is skipped until the studio restarts.',
    computeBackendHint_cuda: 'NVIDIA only, and the fastest there. Needs cuBLAS, downloaded once on the first start.',
    computeBackendHint_vulkan: 'For AMD and Intel cards, experimental; nothing to download. Not for NVIDIA: the DiT on Vulkan hangs an NVIDIA card until Windows resets its driver - choose CUDA there.',
    ...
    customSetPicker: 'Only pinned minimaxmusic.cpp GGUF components are listed. Choose one item in every category; the backend performs final compatibility validation before it starts any download.',
    ...
    captionStructuredHint: 'MiniMax Music 3 reads the description as a labelled document, not a one-line prompt. Roughly 250-400 words in total.',
    ...
    agentCopy: 'Copy',
    agentCopied: 'Copied',
    agentAssistantHint: 'Pick “Agent (MCP)” as the writing assistant (Settings, Models) and the agent writes instead of the local model: the write buttons, and the lyric layout of a dataset preparation, wait for its answers.',
    ...
    cloudSectionHint: 'OpenRouter key and which capability runs where',
