...
async fn wait_for(args: &Value) -> Value {
    ...
    loop {
        let (done, now) = match &job {
            Some(job) => {
                let state = compact_job(&fetch(&format!("/v1/music/jobs/{}", segment(job))).await);
...
fn tools() -> &'static [Tool] {
    ...
    TOOLS.get_or_init(|| {
        vec![
            ...
            Tool {
                name: "studio_status",
                description: "What the studio is doing now, in one short summary: song jobs, covers and karaoke, the dataset preparation and its stages, the training run with its step and loss, audio processing, and whether the studio's window is open. Call it first, and use studio_wait to wait.",
                schema: nothing,
