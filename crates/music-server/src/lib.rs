...
fn titled(request: &CreateMusicJobRequest, library: &adapters::AdapterLibrary) -> String {
    request
        ...
        .unwrap_or_else(|| {
            ...
            auto_title::auto_title(&auto_title::without_triggers(&request.caption, &triggers), &request.lyrics, request.lyrics.trim().is_empty())
        })
}
...
/// The studio's separator, for the vocals lyric recognition hears.
...
/// models run on when nothing else has brought it yet.
...
async fn describe_training_item(
    ...
    if matches!(state.assistant.read().await.provider, AssistantProvider::Managed) {
        card_free_of_training(&state, "describe songs").await?;
    }
...
/// One question to the writing assistant and its answer as text; a schema,
/// when given, holds a local model to that shape of JSON.
...
{tail}") };
    ...
    assistant::content_of(&response).map_err(|error| {
...
async fn create_music_job(
    ...
    if state.training.active_run().await.is_some() {
        return (StatusCode::CONFLICT, Json(failed_request_job(request, engine_id, "a LoRA is training on the card; make songs once it finishes".into())));
    }
...
async fn replay_music_job(
    ...
    if state.training.active_run().await.is_some() {
        return Err(api_error(StatusCode::CONFLICT, "a LoRA is training on the card; replay once it finishes".into()));
    }
    ...
    if let Some(song_id) = &request.song_id {
        if let Some(original) = state.library.get_song(song_id).map_err(|error| api_error(StatusCode::INTERNAL_SERVER_ERROR, error.to_string()))? {
            job.derived = Some(derivation(&original, "replay", serde_json::json!({ "steps": request.steps, "seed": request.seed, "dit_cfg": request.dit_cfg, "output_format": request.output_format })));
        }
    }
    state.jobs.write().await.insert(job.id.clone(), job.clone());
    Ok((StatusCode::ACCEPTED, Json(job)))
...
async fn import_completed_mm_result(state: &AppState, job: &MusicJob, job_id: &str) -> anyhow::Result<Vec<CompletedSong>> {
    ...
    for track in tracks {
        ...
        let imported_song = state.library.import_generated_song(library::GeneratedSongInput {
            ...
            audio_extension: extension, audio,
        })?;
