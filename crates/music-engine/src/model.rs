//! What the adapter library asks of MiniMax Music 3: the parts of the model an
//! adapter changes, the sizes it comes in and what an adapter file holds.

pub use crate::mm_server::{AdapterSlot, ADAPTER_SLOTS};
pub use music_core::adapter_weights::AdapterWeights;
use music_core::adapter_weights::{adapter_format, tensor_names};
use serde_json::Value;

/// MiniMax Music 3 comes in one size, so an adapter's width says nothing to
/// choose by.
...
/// minimaxmusic.cpp merges LoRA and LoKr and refuses DoRA in any form (its adapter.h).
